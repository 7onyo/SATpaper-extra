...
def parse_cnf_file(filepath):
    """Parse a CNF file in DIMACS format and return clauses and number of variables."""
    clauses = []
    num_variables = 0
...
def walksat(clauses, num_variables, max_flips, p=0.5):
    """
    WalkSAT algorithm with frozenset clauses for better memory tracking.
    Returns: (solution, status) where solution is None for UNSAT, and status is 'SAT' or 'UNSAT'.
    """
...
def test_all_cnf_files(folder_path, output_csv, max_flips=10000, p=0.5):
    """Process all CNF files in folder and write results to CSV, matching resolution script format."""
